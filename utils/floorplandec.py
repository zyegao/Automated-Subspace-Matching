...
def run(FLAGS, cfg, image_dir="./segresult/floorplan.jpg", output_dir="./segresult/dec/"):
    # build trainer
    ...
    images = get_test_images(None, image_dir)
...
def main(config_path=None, model_path=None):
    FLAGS = parse_args()
    #config = "./configs/ppyoloe_plus_crn_l_30e_voc.yml"
    #print(FLAGS.config)
    #选择算法使用到的配置文件
    image_dir = "./segresult/floorplan.jpg"
    output_dir = "./segresult/dec/"
    if not isinstance(config_path,type(None)):
        image_dir = "../segresult/floorplan.jpg"
        output_dir = "../segresult/dec/"
        FLAGS.config = config_path
    else:
        FLAGS.config = "./configs/ppyoloe_plus_crn_l_30e_voc.yml"
    cfg = load_config(FLAGS.config)
    #选择目标检测使用的模型文件，该模型使我们自己训练出来的
    if not isinstance(model_path,type(None)):

        cfg.weights = model_path
    else:
        cfg.weights = "./model/detection/best_model.pdparams"
    #cfg.weight = "./model/detection/best_model.pdparams"
    ...
    return run(FLAGS, cfg, image_dir, output_dir)
